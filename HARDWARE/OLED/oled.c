...
void oled_sda_pin_mode(GPIOMode_TypeDef mode)
{
	GPIO_InitStructure.GPIO_Mode=mode;//输入/输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_10;//第10号引脚
	GPIO_InitStructure.GPIO_OType = GPIO_OType_OD;//推挽输出类型
...
void IIC_Start(void)
{
	oled_sda_pin_mode(GPIO_Mode_OUT);
...
void IIC_Wait_Ack(void)
{
	...
	oled_sda_pin_mode(GPIO_Mode_IN);
	...
	unsigned char c=0,i=0;	
		...
		if(Char_Size ==16)
			...
			OLED_WR_Byte(F8X16[c*16+i+8],OLED_DATA);
				...
				OLED_Set_Pos(x,y);
				for(i=0;i<6;i++)
				OLED_WR_Byte(F6F8[c][i],OLED_DATA);
